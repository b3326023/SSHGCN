...
class Data:
    ...
    def get_sentiment_data(self):
        ...
        adj_matrix_s = np.zeros([self.user_size, self.user_size * 2], dtype=np.float32)
        ...
        return self.x_train['holder_id'].values, self.x_train['target_id'].values, self.x_test['holder_id'].values, self.x_test['target_id'].values, X1_s_train, X2_s_train, self.y_train, X1_s_test, X2_s_test, self.y_test, adj_matrix_s[:, :adj_matrix_s.shape[0]]

    def get_complete_adjacency(self):
        # construct sentiment adjacency matrix from train data
        adj_matrix_s = np.zeros([self.user_size, self.user_size * 2], dtype=np.int8)
    ...
    def get_sentiment_pair_data(self):
        # construct sentiment adjacency matrix from train data
        all_user_true_sentiment = np.zeros([12814, 1723], dtype=np.int8)
        for i, row in self.sentiment.iterrows():
            holder = row['holder_id']
            target = row['target_id']
            label = row['label']
    ...
    def get_relation_data(self):
        ...
        adj_matrix_r = np.zeros([self.user_size, self.user_size * 2], dtype=np.float32)
        ...
        return X1_r_train, X2_r_train, X1_r_test, X2_r_test, adj_matrix_r[:, :adj_matrix_r.shape[0]]
        ...
        for index, row in feature.iterrows():
            ...
            for value in values:
                if value != '未知' and value not in dictionary:
                    ...
                    cnt += 1
        one_hot_matrix = np.zeros([self.user_size, len(dictionary)], dtype=np.int8)
